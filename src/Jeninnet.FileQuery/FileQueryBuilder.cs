...
public sealed class FileQueryBuilder {
    ...
    public IEnumerable<string> Execute() => GetEngine().Execute(Build());
    ...
    public IAsyncEnumerable<string> ExecuteAsync(CancellationToken cancellationToken = default) => GetEngine().ExecuteAsync(Build(), cancellationToken);
    ...
    private IFileQueryEngine GetEngine() => _engine ??= FileQueryRuntime.Create();
